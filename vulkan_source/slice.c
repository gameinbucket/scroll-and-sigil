...
slice slice_push_pointer(const slice a, void *const b) {
    slice_head *head = slice_get_head(a);
...
// slice slice_push(const slice a, void *const reference, size_t member_size) {
//     slice_head *head = slice_get_head(a);
//     size_t length = head->length + 1;
//     if (length > head->capacity) {
//         head = slice_resize(head, sizeof(member_size), length);
//         head->capacity = length;
//     }
//     head->length = length;
//     slice data = (slice_head *)head + 1;
//     memcpy(data + (length - 1), reference, member_size);
//     return data;
// }
