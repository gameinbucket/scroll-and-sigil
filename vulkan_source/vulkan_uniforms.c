...
void vk_create_descriptor_set_layout(vulkan_state *vk_state, struct vulkan_renderer *vk_renderer) {
    ...
    if (vkCreateDescriptorSetLayout(vk_state->vk_device, &layout_info, NULL, &vk_renderer->vk_descriptor_set_layout) != VK_SUCCESS) {
...
void vk_create_uniform_buffers(vulkan_state *vk_state, struct vulkan_renderer *vk_renderer) {
    ...
    vk_renderer->uniforms->vk_uniform_buffers = safe_calloc(count, sizeof(VkBuffer));
    vk_renderer->uniforms->vk_uniform_buffers_memory = safe_calloc(count, sizeof(VkDeviceMemory));
    ...
    for (uint32_t i = 0; i < count; i++) {
        vk_create_buffer(vk_state, size, usage, properties, &vk_renderer->uniforms->vk_uniform_buffers[i], &vk_renderer->uniforms->vk_uniform_buffers_memory[i]);
    }
}
...
void vk_update_uniform_buffer(vulkan_state *vk_state, struct vulkan_renderer *vk_renderer, uint32_t current_image, struct uniform_buffer_object ubo) {
    ...
    void *data;
    vkMapMemory(vk_state->vk_device, vk_renderer->uniforms->vk_uniform_buffers_memory[current_image], 0, sizeof(ubo), 0, &data);
    memcpy(data, &ubo, sizeof(ubo));
    vkUnmapMemory(vk_state->vk_device, vk_renderer->uniforms->vk_uniform_buffers_memory[current_image]);
}
...
void vk_create_descriptor_pool(vulkan_state *vk_state, struct vulkan_renderer *vk_renderer) {
    ...
    if (vkCreateDescriptorPool(vk_state->vk_device, &pool_info, NULL, &vk_renderer->vk_descriptor_pool) != VK_SUCCESS) {
...
void vk_create_descriptor_sets(vulkan_state *vk_state, struct vulkan_renderer *vk_renderer) {
    ...
    for (uint32_t i = 0; i < size; i++) {
        memcpy(&descriptor_set_layouts[i], &vk_renderer->vk_descriptor_set_layout, sizeof(VkDescriptorSetLayout));
    }

    VkDescriptorSetAllocateInfo alloc_info = {0};
    alloc_info.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    alloc_info.descriptorPool = vk_renderer->vk_descriptor_pool;
    alloc_info.descriptorSetCount = size;
    alloc_info.pSetLayouts = descriptor_set_layouts;

    vk_renderer->vk_descriptor_sets = safe_calloc(size, sizeof(VkDescriptorSet));

    if (vkAllocateDescriptorSets(vk_state->vk_device, &alloc_info, vk_renderer->vk_descriptor_sets) != VK_SUCCESS) {
    ...
    for (uint32_t i = 0; i < size; i++) {
        ...
        buffer_info.buffer = vk_renderer->uniforms->vk_uniform_buffers[i];
        buffer_info.offset = 0;
        buffer_info.range = sizeof(struct uniform_buffer_object);

        VkDescriptorImageInfo image_info = {0};
        image_info.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        image_info.imageView = vk_renderer->vk_texture_image_view;
        image_info.sampler = vk_renderer->vk_texture_sampler;

        VkWriteDescriptorSet descriptor_write_uniform = {0};
        descriptor_write_uniform.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        descriptor_write_uniform.dstSet = vk_renderer->vk_descriptor_sets[i];
        ...
        descriptor_write_sampler.dstSet = vk_renderer->vk_descriptor_sets[i];
