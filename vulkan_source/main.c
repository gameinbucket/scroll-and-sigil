...
static void window_init(SDL_Window **win, vulkan_state *vk_state) {
    ...
    // vk_state->pipeline2d = pipeline2d;

    struct vulkan_renderer *renderer = safe_calloc(1, sizeof(struct vulkan_renderer));
    vk_state->renderer = renderer;
    ...
    renderer->pipeline = pipeline3d;

    vk_create_state(vk_state);
    vk_create_renderer(vk_state, renderer, SCREEN_WIDTH, SCREEN_HEIGHT);

    *win = window;
}

static void draw(SDL_Window *window, vulkan_state *vk_state, struct vulkan_renderer *vk_renderer) {
...
static void main_loop(SDL_Window *window, vulkan_state *vk_state) {
    ...
    while (run) {
        ...
        log("draw. ");
        draw(window, vk_state, vk_state->renderer);
        sleep_ms(16);
    }
