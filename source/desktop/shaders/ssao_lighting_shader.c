...
struct ssao_lighting_shader *new_ssao_lighting_shader(vulkan_state *vk_state, vulkan_base *vk_base, vulkan_offscreen_buffer *offscreen) {
    ...
    struct ssao_lighting_shader *shader = safe_calloc(1, sizeof(struct ssao_lighting_shader));
    ...
    shader->uniforms3 = new_vulkan_uniform_buffer(sizeof(struct uniform_ssao_lighting));
...

    {
        ...
        memset(bindings, 0, 4 * sizeof(VkDescriptorSetLayoutBinding));
    ...
    pipe_settings.fragment = "shaders/spv/ssao_lighting.frag.spv";
