...
void graphics_draw_range(const void *start, GLsizei count) {
    if (count == 0) {
        return;
    }
...
void graphics_make_vao(renderbuffer *b) {
    ...
    if (position > 0) {
        glVertexAttribPointer(index, position, GL_FLOAT, false, stride, offset);
    ...
    if (color > 0) {
        glVertexAttribPointer(index, color, GL_FLOAT, false, stride, offset);
        glEnableVertexAttribArray(index);
        index++;
        offset += color * 4;
    }
    ...
    if (texture > 0) {
        glVertexAttribPointer(index, texture, GL_FLOAT, false, stride, offset);
        glEnableVertexAttribArray(index);
    }
}
